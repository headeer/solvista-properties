 ...
 */
function oceanwp_register_property_search_map_template($templates)
{
	$templates['page-templates/property-search-map.php'] = __('Property Search Map', 'oceanwp');
	return $templates;
}
add_filter('theme_page_templates', 'oceanwp_register_property_search_map_template');

/**
 * Enqueue scripts and styles for property search map
 */
function oceanwp_enqueue_property_search_map_assets()
{
	// Only enqueue on pages using the property search map template
	if (is_page_template('page-templates/property-search-map.php')) {
		// Font Awesome (already included in OceanWP)
		...
		wp_enqueue_style(
			'property-search-map',
			OCEANWP_THEME_URI . '/page-templates/property-search-map.css',
			array('leaflet'),
		...
		wp_enqueue_script(
			'property-search-map',
			OCEANWP_THEME_URI . '/page-templates/property-search-map.js',
...
add_action('wp_enqueue_scripts', 'oceanwp_enqueue_property_search_map_assets');

/**
 * Register Property Custom Post Type
 */
function oceanwp_register_property_post_type()
{
	$labels = array(
		'name' => __('Properties', 'oceanwp'),
		'singular_name' => __('Property', 'oceanwp'),
		'menu_name' => __('Properties', 'oceanwp'),
		'add_new' => __('Add New', 'oceanwp'),
		'add_new_item' => __('Add New Property', 'oceanwp'),
		'edit_item' => __('Edit Property', 'oceanwp'),
		'new_item' => __('New Property', 'oceanwp'),
		'view_item' => __('View Property', 'oceanwp'),
		'search_items' => __('Search Properties', 'oceanwp'),
		'not_found' => __('No properties found', 'oceanwp'),
		'not_found_in_trash' => __('No properties found in Trash', 'oceanwp'),
	);
...
add_action('init', 'oceanwp_register_property_post_type');

/**
 * Register REST API endpoint for properties
 */
function oceanwp_register_properties_endpoint()
{
	register_rest_route('solvista/v1', '/properties', array(
		'methods' => 'GET',
		'callback' => 'oceanwp_get_properties',
		'permission_callback' => '__return_true'
	));
}
add_action('rest_api_init', 'oceanwp_register_properties_endpoint');

/**
 * Get properties based on filters
 */
function oceanwp_get_properties($request)
{
...
/**
 * Register REST API endpoint for getting search by map properties
 */

add_action('rest_api_init', function () {
    register_rest_route('resales/v1', '/map-properties', [
        'methods'  => 'GET',
        'callback' => 'get_resales_properties',
        'permission_callback' => '__return_true',
    ]);
});

/**
 * Get reseale properties based for search by map
 */

 function get_resales_properties() {
    error_log("=== Resales API Request Started ===");

    $result = [];
    $ch = curl_init();

	curl_setopt_array($ch, array(
		CURLOPT_URL => 'https://webapi.resales-online.com/V6/SearchProperties?P_PageSize=100&p_agency_filterid=5&p1=1033180&p2=549e6da9b048b768a21e0f82ff3b019566fb164a',
    ...
    if (curl_errno($ch)) {
        ...
        curl_close($ch);
        return new WP_Error('api_error', 'Unable to fetch property list', ['status' => 500]);
    }
    ...
    if (isset($listData['Property']) && is_array($listData['Property'])) {
        foreach ($listData['Property'] as $item) {
            $reference = $item['Reference'];
            error_log("Processing reference: $reference");

            $propertyUrl = "https://webapi.resales-online.com/V6/PropertyDetails?p_agency_filterid=5&p1=1033180&p2=549e6da9b048b768a21e0f82ff3b019566fb164a&P_RefId={$reference}&P_ShowGPSCoords=TRUE";
            ...
            $propertyJson = curl_exec($ch);

            if (curl_errno($ch)) {
                error_log("cURL Error (detail): " . curl_error($ch));
                curl_close($ch);
                continue;
            }
            ...
            $propertyData = json_decode($propertyJson, true);
            ...
            if (isset($propertyData['Property'])) {
                ...
                $result[] = [
                    ...
                    'gps_x'     => $property['GpsX'] ?? null,
                    'gps_y'     => $property['GpsY'] ?? null,
                    'price'     => $property['Price'] ?? null,
                    'province'     => $property['Province'] ?? null,
                    'area'     => $property['Area'] ?? null,
                    'location'     => $property['Location'] ?? null,
                    'pictureUrl'     => $property['Pictures']['Picture'][0]['PictureURL'] ?? null,
                ];

                error_log("Added property: " . $property['Reference']);
            } else {
                error_log("No property data found for: $reference");
            }
        }
    } else {
        error_log("No properties found in list.");
    }

    error_log("=== Resales API Request Completed ===");
